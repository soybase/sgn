...
##LABEL	ITC	GENOTYPE_NAME_ORIGINAL	GENOTYPE	SYNONYM1	SYNONYM2	SYNONYM3	SYNONYM4	PLOIDY	GERMPLASM_GROUP	CATEGORY	INSTITUTE	LOCATION	LOCATION_CODE	CURATOR

my ($new_count,$existing, $count, $syn_count, $merge);
my $coderef= sub  {
    foreach my $num (@rows ) {
	my $accession = $spreadsheet->value_at($num,'GENOTYPE'); 
	if (!$accession) { next; }
	...
	my $organization = $spreadsheet->value_at($num, 'INSTITUTE');
        ...
        my $ITC_name = $spreadsheet->value_at($num, "ITC");
        my $original_name = $spreadsheet->value_at($num, "GENOTYPE_NAME_ORIGINAL");
        my $syn1 =  $spreadsheet->value_at($num, "SYNONYM1");
		...
		my $genome_structure = $spreadsheet->value_at($num, "GERMPLASM_GROUP");
	...
	$population->find_or_create_related('stock_relationship_objects', {
        ...
        my @synonyms = ($accession,$syn1, $syn2, $syn3, $syn4);
